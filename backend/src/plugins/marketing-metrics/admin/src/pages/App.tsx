/* ──────────────────────────────────────────────────────────────────────────────
  SEO & Analytics — Strapi Admin Plugin UI (Vercel-grade light mode + Motion polish)
  - Dark mode: érintetlen (a meglévő DARK_TOKENS marad)
  - Light mode: “soft surface system” + finom outline + 2-layer shadow
  - Framer Motion: hover/press a row-okon + Tab pill anim + Drawer anim + ESC close
────────────────────────────────────────────────────────────────────────────── */
...
interface QueryRow {
  ...
  clicks: number;
  impressions: number;
  position: number | string; // API néha stringként is visszaadhatja → védjük magunkat
}
...
const SOURCES = [
  {
    key: 'pages',
    label: 'Pages',
    endpoint: '/api/pages?pagination%5BpageSize%5D=100&populate=seo',
    titleField: 'slug',
    single: false,
    uid: 'api::page.page',
  },
  {
    key: 'articles',
    label: 'Articles',
    endpoint: '/api/articles?pagination%5BpageSize%5D=100&populate=seo',
    titleField: 'title',
    single: false,
  ...
  },
  {
    key: 'products',
    label: 'Products',
    endpoint: '/api/products?pagination%5BpageSize%5D=100&populate=seo',
    titleField: 'title',
    single: false,
  ...
  },
  {
    key: 'blog-page',
    label: 'Blog page',
    endpoint: '/api/blog-page?populate=seo',
    titleField: 'heading',
    single: true,
  ...
  },
  {
    key: 'global',
    label: 'Global',
    endpoint: '/api/global?populate=seo',
    titleField: '_fixed_Global settings',
    single: true,
...
const normalizeGscData = (
  raw: Record<string, GscPageData>
): Record<string, GscPageData> => {
...
const MOCK_GSC_DATA: Record<string, GscPageData> = {
  ...
  '/csontkovacs-budapest': {
    url: '/csontkovacs-budapest',
    clicks: 21,
    impressions: 510,
    ctr: 0.041,
    position: 2.9,
...
const MOCK_TOP_PAGES: TopPage[] = [
  ...
  {
    url: 'https://davelopment.hu/homepage',
    slug: '/homepage',
    clicks: 38,
    impressions: 870,
    position: 1.4,
  ...
  {
    url: 'https://davelopment.hu/csontkovacs-budapest',
    slug: '/csontkovacs-budapest',
    clicks: 21,
    impressions: 510,
    position: 2.9,
  },
  {
    url: 'https://davelopment.hu/szolgaltatasok',
    slug: '/szolgaltatasok',
    clicks: 14,
    impressions: 380,
    position: 4.5,
  ...
  {
    url: 'https://davelopment.hu/projektek',
    slug: '/projektek',
    clicks: 8,
    impressions: 220,
    position: 7.1,
...
const DARK_TOKENS: Record<string, string> = {
  ...
  '--shadow-hover':
    '0 0 0 1px rgba(240,246,252,0.06), 0 8px 28px rgba(0,0,0,0.55)',
  '--chart-green': '#3dffa0',
  '--chart-indigo': '#7c6af7',
  '--chart-green-raw': '#3dffa0',
  '--chart-indigo-raw': '#7c6af7',
};

/* ──────────────────────────────────────────────────────────────
   LIGHT TOKENS — Vercel-grade “soft surface system”
   CÉL:
   - page bg: marad #f5f5f5
   - cards: off-white (ne vakító #fff)
   - 3 szint: page / card / inner
   - border: soft outline
   - shadow: 2-layer elevation + hover shadow külön
───────────────────────────────────────────────────────────────────────────── */
const LIGHT_TOKENS: Record<string, string> = {
  ...
  // Soft outline borders (nem “keret”, inkább “kontúr”)
  '--border': 'rgba(15, 23, 42, 0.10)',
  '--border-hover': 'rgba(15, 23, 42, 0.16)',

  // Text hierarchy (primary tiszta, a többi “quiet”)
  '--text-primary': '#0b1220',
  '--text-secondary': '#111827',
  '--text-muted': 'rgba(17, 24, 39, 0.62)',
  '--text-faint': 'rgba(17, 24, 39, 0.46)',
  '--text-faintest': 'rgba(17, 24, 39, 0.28)',

  // Accents (megtartva, lighthoz hangolva)
  '--accent-green': '#1a7f37',
  '--accent-red': '#cf222e',
  '--accent-amber': '#9a6700',
  '--accent-indigo': '#6639ba',
  '--accent-indigo-light': '#7c3aed',

  // Vercel-ish 2-layer elevation
  '--shadow':
    '0 0 0 1px rgba(15, 23, 42, 0.06), 0 6px 22px rgba(15, 23, 42, 0.06)',
  '--shadow-hover':
    '0 0 0 1px rgba(15, 23, 42, 0.08), 0 10px 30px rgba(15, 23, 42, 0.10)',

  // Charts
  '--chart-green': '#1a7f37',
  '--chart-indigo': '#6639ba',
  '--chart-green-raw': '#1a7f37',
  '--chart-indigo-raw': '#6639ba',
};
...
function getStrapiTheme(): 'light' | 'dark' {
  ...
  if (
    ...
    html.classList.contains('strapi-theme-light')
  )
    return 'light';
  if (
    html.classList.contains('dark-theme') ||
    html.classList.contains('strapi-theme-dark')
  )
    return 'dark';
...
const staticStyles = `
  /* ──────────────────────────────────────────────────────────────
     FONT
     - Geist Mono: meta/tech UI-ra (slug, date, table header) prémium.
  ────────────────────────────────────────────────────────────── */
  @import url('https://fonts.googleapis.com/css2?family=Geist+Mono:wght@400;500&display=swap');

  /* ──────────────────────────────────────────────────────────────
     GLOBAL BASE
     - box-sizing: stabil layout
     - focus-visible: billentyű UX (szép, de nem zavaró egérrel)
  ────────────────────────────────────────────────────────────── */
  ...
  /* ──────────────────────────────────────────────────────────────
     AUDIT ROW (legacy class — a motion wrapperrel együtt dolgozik)
     MIÉRT: a motion csak mozgat, a “look” itt van.
  ────────────────────────────────────────────────────────────── */
  .audit-row {
  ...
  /* ──────────────────────────────────────────────────────────────
     TOOLTIP (quiet, premium)
     - blur + soft shadow: Vercel jelleg
  ────────────────────────────────────────────────────────────── */
  ...
  /* ──────────────────────────────────────────────────────────────
     CHART DRAW
  ────────────────────────────────────────────────────────────── */
  @keyframes chart-draw { from { stroke-dashoffset: var(--dash-total); } to { stroke-dashoffset: 0; } }
  .chart-line-animated {
  ...
  /* ──────────────────────────────────────────────────────────────
     STAT CARD (lightban a shadow + lift adja a prémium érzetet)
  ────────────────────────────────────────────────────────────── */
  .stat-card {
    background: var(--bg-card-sm);
    border: 1px solid var(--border);
  ...
  /* ──────────────────────────────────────────────────────────────
     TAB BUTTONS (a mozgó pill háttér motionből jön)
  ────────────────────────────────────────────────────────────── */
  .tab-btn {
  ...
  /* ──────────────────────────────────────────────────────────────
     LAYOUT TOGGLE BUTTONS (icon button)
  ────────────────────────────────────────────────────────────── */
  .layout-btn {
    display: flex; align-items: center; justify-content: center;
    width: 32px; height: 32px; border-radius: 9px; cursor: pointer;
    border: 1px solid var(--border); background: transparent;
    color: var(--text-muted); transition: background 140ms ease, border-color 140ms ease, color 140ms ease, transform 140ms ease;
    flex-shrink: 0;
  }
  .layout-btn:hover {
    border-color: var(--border-hover);
  ...
  /* ──────────────────────────────────────────────────────────────
     TABLE LAYOUT
     - soft separators, header inner surface
  ────────────────────────────────────────────────────────────── */
  .audit-table {
    background: var(--bg-card);
    border: 1px solid var(--border);
  ...
  .audit-table-row:not(:last-child) { border-bottom: 1px solid rgba(15, 23, 42, 0.06); }
...
const auditItem = (
  ...
  return {
    ...
    type: typeLabel,
    label,
    slug: item.slug || (single ? '' : '/'),
    issues,
    score,
    editUrl: getEditUrl(uid, single, item.documentId),
  };
};
      ...
      <div style={{ display: 'flex', gap: '16px', flexShrink: 0 }}>
        {['klikk', 'megj.', 'pozíció'].map(l => (
          <div key={l} style={{ textAlign: 'right', minWidth: '40px' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--text-faintest)' }}>
              {l}
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ display: 'flex', gap: '16px', flexShrink: 0 }}>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        >
          megj.
          <InfoTip text="Hányszor jelent meg ez az oldal a Google találatokban." />
        </div>
      </div>
...
// ──────────────────────────────────────────────────────────────────────────────
// Motion helpers (row hover/tap — Vercel-grade)
// ──────────────────────────────────────────────────────────────────────────────
const MotionRow = ({
  className,
  style,
...
// ──────────────────────────────────────────────────────────────────────────────
// Tab nav — moving pill background (layoutId) for Vercel feel
// ──────────────────────────────────────────────────────────────────────────────
const TabNav = ({
  activeTab,
  setActiveTab,
  ...
  setActiveTab: (k: 'audit' | 'analytics') => void;
}) => {
  const tabs = [
    { k: 'audit' as const, l: 'SEO Audit' },
...
const AuditTable = ({
  ...
  <div className="audit-table">
    <div className="audit-table-header">
      {['', 'Oldal', 'Típus', 'Hiányzik', 'Klikk', 'Megj.', 'Poz.', ''].map(
        (h, i) => (
          <div
            key={i}
            style={{
    ...
    {items.map(item => {
      ...
      return (
        <MotionRow key={item.id} className="audit-table-row">
          <div>
            ...
            <>
              <div style={{ padding: '0 4px', textAlign: 'right' }}>
                <span
                  style={{
                    fontSize: '13px',
                    fontWeight: 700,
                    color: gsc ? 'var(--chart-green)' : 'var(--text-faintest)',
                    fontVariantNumeric: 'tabular-nums',
                  }}
                >
                  {gsc ? fmt(gsc.clicks) : '—'}
              ...
              <div style={{ padding: '0 4px', textAlign: 'right' }}>
                <span
                  style={{
                    fontSize: '12px',
                    color: gsc ? 'var(--text-muted)' : 'var(--text-faintest)',
                    fontVariantNumeric: 'tabular-nums',
                  }}
                >
                  {gsc ? fmt(gsc.impressions) : '—'}
                </span>
              </div>
          ...
          <div style={{ padding: '0 0 0 4px' }}>
            <button
              onClick={() => (window.location.href = item.editUrl)}
              style={{
                padding: '4px 10px',
                borderRadius: '8px',
                fontSize: '11px',
        ...
        >
          ...
          <button
            ...
            onMouseEnter={e => {
              e.currentTarget.style.borderColor = 'var(--border-hover)';
              e.currentTarget.style.color = 'var(--text-primary)';
              e.currentTarget.style.background = 'var(--bg-inner2)';
            }}
            onMouseLeave={e => {
              e.currentTarget.style.borderColor = 'var(--border)';
              e.currentTarget.style.color = 'var(--text-muted)';
...
const VercelChart = ({ trend, metric, isDark, height = 160 }: ChartProps) => {
  ...
  const buildPath = (vals: number[], s: ReturnType<typeof scale>) =>
    vals
      .map(
        (v, i) =>
          `${i === 0 ? 'M' : 'L'}${px(i).toFixed(2)},${pyFor(v, s).toFixed(2)}`
      )
      .join(' ');

  const buildArea = (vals: number[], s: ReturnType<typeof scale>) => {
    const line = buildPath(vals, s);
    return `${line} L${px(vals.length - 1).toFixed(2)},${(
      PAD.top + iH
    ).toFixed(2)} L${PAD.left.toFixed(2)},${(PAD.top + iH).toFixed(2)} Z`;
  };
      ...
      >
        ...
        {yTicks.map((tick, i) => (
          <g key={i}>
            <line
              x1={PAD.left}
              y1={pyFor(tick, primaryScale)}
              x2={PAD.left + iW}
              y2={pyFor(tick, primaryScale)}
              stroke="var(--border)"
              strokeWidth="1"
        ...
        {showImps && mounted && (
          <>
            <path
              d={buildArea(impVals, iScale)}
              fill="url(#grad-i)"
              clipPath="url(#chart-clip)"
              className="chart-area-animated"
            />
            <path
              d={buildPath(impVals, iScale)}
              fill="none"
              ...
              style={{ '--dash-total': iLen } as React.CSSProperties}
            />
          </>
        )}
        ...
        {showClicks && mounted && (
          <>
            <path
              d={buildArea(clickVals, cScale)}
              fill="url(#grad-c)"
              clipPath="url(#chart-clip)"
              className="chart-area-animated"
            />
            <path
              d={buildPath(clickVals, cScale)}
              fill="none"
              ...
              clipPath="url(#chart-clip)"
              className="chart-line-animated"
              style={{ '--dash-total': cLen } as React.CSSProperties}
            />
          </>
        )}
        ...
        {tooltip && (
          <>
            ...
            {showClicks && (
              <circle
                cx={tooltip.x}
                cy={pyFor(trend[tooltip.idx].clicks, cScale)}
                r="4"
            ...
            {showImps && (
              <circle
                cx={tooltip.x}
                cy={pyFor(trend[tooltip.idx].impressions, iScale)}
                r="4"
...
// ─── Query Drawer (AnimatePresence + ESC close + safe types) ───────────────────
const QueryDrawer = ({
  slug,
  onClose,
  isMock,
  ...
  // ESC close + minimal focus UX (nyitáskor close gomb fókusz)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const res = await fetch('/api/marketing-metrics/gsc-queries', {
          method: 'POST',
        ...
        const rows = Array.isArray(d?.queries) ? (d.queries as QueryRow[]) : [];
        setQueries(rows);
      } catch {
        if (!alive) return;
        setQueries([]);
      ...
      >
        <motion.div
          ...
          style={{
            ...
            padding: '24px',
            overflowY: 'auto',
            boxShadow: '-18px 0 55px rgba(0,0,0,0.18)',
          }}
        ...
        >
          ...
          {loading ? (
            ...
            queries.map((q, i) => {
              const position =
                typeof q.position === 'number' ? q.position : Number(q.position);
                    ...
                    <div>
                      <div
                        style={{
                          fontSize: '14px',
                          fontWeight: 800,
                          color: Number.isFinite(position)
                            ? posColor(position)
                            : 'var(--text-faint)',
                          letterSpacing: '-0.3px',
                        }}
...
export const App = () => {
        ...
        fetch(src.endpoint)
          ...
          .then(data => {
            ...
            return (data.data || []).map((it: any) =>
              auditItem(it, src.titleField, src.label, src.uid, false)
            );
          })
          .catch(() => [])
      )
  ...
  useEffect(() => {
    ...
    fetch('/api/marketing-metrics/gsc-status')
      ...
      .then(d => {
        ...
        if (d.connected) loadAllGscData(false);
      })
      .catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const loadAllGscData = useCallback(
    async (mock: boolean = isMock) => {
      setGscLoading(true);
      try {
        if (mock) {
          ...
          const [pageRes, trendRes, topRes] = await Promise.all([
            fetch('/api/marketing-metrics/gsc-data', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({}),
            }),
  ...
  useEffect(() => {
    if (gscConnected) loadAllGscData();
  }, [trendDays, gscConnected, loadAllGscData]);
  ...
  const avgPos = Object.values(gscData).length
    ? Object.values(gscData).reduce((a, b) => a + b.position, 0) / Object.values(gscData).length
    : 0;
        ...
        >
              ...
              {IS_DEV && (
                <span
                  ...
                  style={{
                    ...
                    background: 'rgba(240,199,66,0.1)',
                    border: '1px solid rgba(240,199,66,0.25)',
                    padding: '1px 7px',
                    borderRadius: '20px',
                    fontWeight: 700,
                  }}
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            {gscConnected ? (
              ...
              <button
                onClick={() => (window.location.href = '/api/marketing-metrics/gsc-auth')}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: '6px',
        ...
        >
          {[
            { l: 'Összes', v: items.length, sub: 'tartalom', c: 'var(--text-primary)', tip: '' },
            {
              l: 'Rendben',
              v: ok,
              sub: `${Math.round((ok / Math.max(items.length, 1)) * 100)}%`,
              c: 'var(--accent-green)',
              tip: '',
            },
            ...
            ...(gscConnected
              ? [
                  {
                    l: 'Klikk',
                    v: gscLoading ? '…' : fmt(totalClicks),
                    sub: `${trendDays} nap`,
                    c: 'var(--chart-green)',
                    tip: 'Hányszor kattintottak az oldalaidra a Google keresésből.',
                    change: clicksChange,
                    invert: false,
                  },
                  {
                    l: 'Megjelenés',
                    v: gscLoading ? '…' : fmt(totalImpressions),
                    sub: `${trendDays} nap`,
                    c: 'var(--chart-indigo)',
                    tip: 'Hányszor jelent meg oldalad a Google találati listáján — kattintástól függetlenül.',
                    change: impsChange,
                    invert: false,
                  },
                  {
                    l: 'Átlag pozíció',
                    v: gscLoading || !avgPos ? '…' : `#${avgPos.toFixed(1)}`,
                    sub: 'ranking',
                    c: posColor(avgPos || 99),
                    tip: 'Az oldalaid átlagos Google-helyezése. Minél kisebb, annál jobb.',
                    change: null,
                    invert: true,
                  },
                ]
            ...
            <div key={i} className="stat-card">
              ...
              <div style={{ fontSize: '10px', color: 'var(--text-faint)', marginTop: '1px', fontFamily: "'Geist Mono', monospace" }}>
                {c.sub}
              </div>
            </div>
          ))}
            ...
            {!gscConnected ? (
              <div style={{ ...card, textAlign: 'center', padding: '56px 48px' }}>
                ...
                <div style={{ fontSize: '15px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '6px' }}>
                  Search Console nincs csatlakoztatva
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '16px' }}>
                          {(trendMetric === 'clicks' || trendMetric === 'dual') && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                              ...
                              <span style={{ fontSize: '12px', color: 'var(--chart-green)', fontWeight: 900 }}>
                                {fmt(trend.reduce((a, b) => a + b.clicks, 0))}
                              </span>
                          ...
                          {(trendMetric === 'impressions' || trendMetric === 'dual') && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                              ...
                              <span style={{ fontSize: '12px', color: 'var(--chart-indigo)', fontWeight: 900 }}>
                                {fmt(trend.reduce((a, b) => a + b.impressions, 0))}
                              </span>
                ...
                {/* ── Top pages ── */}
                <div style={card}>
                  <div style={{ fontSize: '13px', fontWeight: 900, color: 'var(--text-primary)', marginBottom: '18px' }}>
                    Top oldalak
                  </div>
                  ...
                  {gscLoading ? (
                    ...
                    <div style={{ color: 'var(--text-faint)', fontSize: '12px', textAlign: 'center', padding: '24px' }}>
                      Nincs adat
                    </div>
                  ) : (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px', padding: '0 2px' }}>
                        ...
                        <span style={{ flex: 1, fontSize: '10px', color: 'var(--text-faint)', fontFamily: "'Geist Mono', monospace" }}>
                          OLDAL
                        </span>
                        <span style={{ fontSize: '10px', color: 'var(--text-faint)', minWidth: '44px', textAlign: 'right' }}>
                          KLIKK
                        </span>
                        <span style={{ fontSize: '10px', color: 'var(--text-faint)', minWidth: '44px', textAlign: 'right' }}>
                          POZÍCIÓ
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        {topPages.map((p, i) => {
                          ...
                          return (
                            ...
                            >
                              <span style={{ fontSize: '11px', color: 'var(--text-faintest)', minWidth: '16px', textAlign: 'right', fontFamily: "'Geist Mono', monospace" }}>
                                {i + 1}
                              </span>

                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontFamily: "'Geist Mono', monospace", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                  {p.slug}
                                </div>
                              ...
                              <div style={{ minWidth: '44px', textAlign: 'right' }}>
                                <div style={{ fontSize: '13px', fontWeight: 900, color: 'var(--chart-green)', fontVariantNumeric: 'tabular-nums' }}>
                                  {fmt(p.clicks)}
                                </div>
                              </div>

                              <div style={{ minWidth: '44px', textAlign: 'right' }}>
                                <div style={{ fontSize: '13px', fontWeight: 900, color: posColor(p.position), fontVariantNumeric: 'tabular-nums' }}>
        ...
        {activeTab === 'audit' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', flexShrink: 0, background: 'var(--bg-inner)', padding: '3px', borderRadius: '10px', border: '1px solid var(--border)' }}>
