...
type NewsletterInput = { type: string; name: string; placeholder?: string | null };
type NewsletterForm = { inputs?: NewsletterInput[] };
...
export const Newsletter: React.FC<NewsletterProps> = ({
  ...
  const genericError = isHu ? 'Hiba történt a feliratkozás közben.' : 'Something went wrong while subscribing.';
  const networkError = isHu ? 'Hálózati hiba történt. Próbáld újra később.' : 'Network error. Please try again later.';
              ...
              >
                <div className="overflow-hidden h-6">
                  <motion.div className="flex flex-col mr-4" variants={wheelVariants}>
                    <span>{submitting ? buttonSending : buttonLabel}</span>
                    <span>{submitting ? buttonSending : buttonLabel}</span>
                  </motion.div>
                </div>
